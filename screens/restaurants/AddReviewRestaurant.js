...
export default function AddReviewRestaurant({ navigation, route}) {
    ...
    const addReview = async() => {
        ...
        if(!responseGetRestaurant.statusResponse){
            setLoading(false)
            toastRef.current.show("Error al obtener el restaurante. Por favor ingresa mas tardes", 2000)
            return
        }
    ...
    const validForm = () => {
        ...
        if(!rating){
            toastRef.current.show("Debes darle una puntuacion al restaurnate", 2000)
            isValid = false
        }
        if(isEmpty(title)){
            setErrorTitle("Debes darle un titulo a tu comentario.")
            isValid = false
        }
        if(isEmpty(review)){
